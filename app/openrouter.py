...
EDIT_PROMPT = (
    ...
    "(3) CRITICAL — straighten the torso until BOTH shoulders are perfectly "
    "level on one horizontal line (same Y height left and right), squared to "
    "the camera; shirt collar and shoulder seam must not slope. "
    "If the selfie has tilted shoulders, fix the posture — this is mandatory. "
    "Do not tilt or rotate the head independently; keep face orientation from the input. "
    ...
    "Only shoulders/posture and background may differ. Remove colour spill / halo on clothes edges."
)
...
GOSUSLUGI_EDIT_PROMPT = (
    ...
    "КРИТИЧНО — ПЛЕЧИ: оба плеча строго на одной горизонтали "
    "(левое и правое на одной высоте, без наклона линии плеч и воротника). "
    "Если на селфи корпус наклонён — обязательно выпрями: плечи ровно, "
    "грудь прямо к камере, как в студии на паспорт. Косой наклон плеч запрещён. "
    "Голову при этом не крути и не наклоняй отдельно: сохрани ориентацию лица с исходника. "
    ...
    "Разрешено только: заменить фон на чисто белый, подчистить контур одежды/плеч "
    "и выровнять плечи в горизонталь / корпус анфас. "
