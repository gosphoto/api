...
GOSUSLUGI_EDIT_PROMPT = (
    ...
    "touch or blend into the white backdrop. Do BOTH, never one without the other: (1) keep "
    "the blazer / jacket lapels so they fully separate the shirt collar from "
    "the background, AND (2) recolor that gray/grey dress shirt to a light-blue "
    "business shirt (голубая деловая) so the collar reads clearly against "
    "#FFFFFF. Always both: lapels separating the collar AND a light-blue shirt. "
    ...
    "do NOT keep pale clothes. Children: never a business suit, never pale "
    "clothes on #FFFFFF. If the child already wears a pink tank, tee, top, or "
    "dress: replace it with a proper decent solid pink child's tank top "
    "(розовая приличная майка) — simple, modest, opaque, no ruffles; not navy, "
    "not a suit, not a pale/white pink that vanishes on #FFFFFF. Other children: "
    "replace with a dark solid child's t-shirt (navy, charcoal, or dark blue). "
    "Adults in a light tank/tee (and not in religious clothing): still the "
    "business suit. "
    ...
    "open — hair must NOT cover eyes, brows, forehead, temples, cheeks, ears, or "
    "the face oval. If the input has hair on or over the face oval: MUST sweep, "
    "part, or tuck that hair away from the oval until the full face and both ears "
    "are clearly visible (behind ears or along shoulders is OK). This face-clearing "
    "move is required even when it means tucking hair behind the ears. "
    "BANGS / FRINGE (чёлка) — CRITICAL for Gosuslugi: if bangs cover, touch, or "
    "sit on the eyebrows OR hide any of the forehead above the brows, you MUST "
    "lightly comb the fringe aside (part to one side or split both sides) until "
    "BOTH eyebrows are fully visible and a strip of forehead skin shows above "
    "them. Leaving bangs over the brows is FORBIDDEN. Keep the same hair color "
    "and overall length — only a small side-sweep of the fringe; do NOT shave "
    "bangs off, do NOT invent a new hairstyle. "
    "Gaze straight into the camera lens; neutral closed-mouth expression — no "
    "smile, no laugh, no grimace (if the input smiles, relax the mouth to neutral "
    ...
    "Do not restyle the hair mass beyond what is needed to clear the face oval. "
    "Do not cut or shorten the main hair, do not change hair color or overall "
    "volume, do not invent a new hairstyle — only reposition hair that was "
    "covering the face. "
    "No watermarks, text, logos, or frames. Photorealistic, high quality."
)
...
GOSUSLUGI_SCORING_PROMPT = (
    ...
    "open face with no hair on the oval (tuck/sweep away if needed); BANGS: if "
    "чёлка covers brows — MUST part fringe aside so BOTH eyebrows and forehead "
    "show (leaving bangs on brows is forbidden); MUST erase "
    "isolated flyaway wisps and leftover wall halo; "
    ...
    "gray/grey shirt collar must not touch #FFFFFF — always both: blazer "
    "lapels separating the collar AND recolor shirt to light-blue business; "
    "children: never a suit; if input already has a pink tank/tee/top → decent "
    "pink tank (розовая приличная майка); else dark navy/charcoal t-shirt — "
    "no pale/white tank or tee on white bg; "
