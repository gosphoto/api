...
GOSUSLUGI_EDIT_PROMPT = (
    ...
    "Then go over the result again carefully — пройди ещё раз внимательно: "
    "separate the gray background from the person. Inspect the contour of the "
    "face, hair, braids, pigtails, ears, and neck. No leftover gray, off-white, "
    "wall, or halo next to the person — only pure #FFFFFF outside the silhouette. "
    "Do not erase hair or braids to do that; do not change face geometry. "
    "If the original background was gray or grey: walk the person's outline "
    "again — если фон был серым, пройдись повторно по контурам человека и "
    "замени остаток серого фона на белый #FFFFFF. Trace face, hair, braids, "
    "ears, neck, shoulders; paint only leftover gray/wall pixels white; "
...
POST_CROP_CLEANUP_PROMPT = (
    ...
    "HARD LOCK on the 35x45 crop: do not zoom, pan, scale, rotate, recrop, or "
    "shift the person even one pixel. Keep the exact head size, head height, "
    "top margin, face position, and silhouette. "
    "Жёстко: не сдвигай голову, не меняй размер лица, не трогай кроп 35×45. "
    "If cleaning the background would change the face or crop, leave the person "
    "untouched and only paint leftover gray/halo to #FFFFFF. "
    "Look very carefully so no gray background remains next to the person — "
    "смотри внимательнее, чтобы серый фон не оставался рядом с лицом, волосами, "
    "косичками человека: inspect the contour of the face, hair mass, braids, "
    "pigtails, flyaways, ears, and neck. Any leftover gray, off-white, wall, "
    "halo, or shadow there must become pure #FFFFFF. Do not erase hair or braids "
    "to do that — only the background pixels. "
...
GOSUSLUGI_SCORING_PROMPT = (
    ...
    "go over again carefully and separate gray background from the person "
    "(face, hair, braids — no leftover gray next to the silhouette); "
    "if the source bg was gray, retrace the person's contours and replace "
    "leftover gray with #FFFFFF; "
