...
EDIT_PROMPT = (
    ...
    "(3) gently straighten the torso so both shoulders are level / horizontal "
    "(same height) and the person faces the camera squarely — do not tilt or "
    "rotate the head independently; keep the face orientation as in the input. "
...
GOSUSLUGI_EDIT_PROMPT = (
    ...
    "ПЛЕЧИ: выровняй оба плеча на одну горизонталь (одинаковая высота), "
    "корпус прямо к камере, без наклона вбок — как в студийном фото на паспорт. "
    ...
    "Разрешено только: заменить фон на чисто белый, слегка подчистить контур одежды/плеч "
    "и выровнять линию плеч/корпус анфас. "
