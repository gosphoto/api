...
- `POST /api/process` — gate → **Gemini Flash Image** (белый фон + костюм) → 35×45 crop (`PASSPORT_CROP_WIDTH_CORR=auto`: окно чуть шире на кадре 3:4, чтобы лицо не выглядело раздутым) → **лист 10×15** → `result_id`. Второй Gemini после кропа (`POST_CROP_CLEANUP`) **выключен**: раздувает овал лица, [замер](docs/post-crop-cleanup-face-widen.md). Fallback — local cutout
